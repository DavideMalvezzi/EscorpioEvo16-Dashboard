...
boolean MotorDriverLoaderClass::initFromEEPROM(){
	...
	for(int i = 0; i < MAX_MAP_NUM; i++){
		...
		valid &= (map[i].ack == getAck((byte*)&map[i], sizeof(MotorMap) - 1));
	}
...
void MotorDriverLoaderClass::onCanRequest(CanFrame* frame){
	if (strstr_P((char*)frame->data, GET_DATA_CMD) != NULL){
		onGetRequest();
	}
	else if (strstr_P((char*)frame->data, SET_MOT_DATA_CMD) != NULL){
		onSetMotorRequest();
	}
	else if (strstr_P((char*)frame->data, SET_MAP_DATA_CMD) != NULL){
